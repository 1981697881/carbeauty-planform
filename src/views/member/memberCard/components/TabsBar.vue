    ...
    <el-form v-model="search" :size="'mini'" :label-width="'80px'">
      <el-row :gutter="10">
        ...
        <!--  <el-button v-for="(t,i) in btnList" :key="i" v-if="t.category == 'default'" :size="'mini'" type="primary" :icon="t.cuicon" @click="onFun(t.path)">{{t.menuName}}</el-button>-->
          ...
          <!--<el-button :size="'mini'" type="primary" icon="el-icon-guide" @click="recharge">充值</el-button>-->
...
import { increaseRecharge } from '@/api/member/index'
  ...
  methods: {
    ...
    Delivery() {
      if (this.clickData.id) {
        this.$confirm('是否删除（' + this.clickData.vipName + '），删除后将无法恢复?', '提示', {
    ...
    recharge() {
      if (this.clickData.id) {
        this.$prompt('请输入充值金额', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          inputPattern: /^(?:[1-9][0-9]*\.[0-9]+|0\.(?!0+$)[0-9]+|0)$/,
          inputErrorMessage: '金额格式不正确'
        }).then(({ value }) => {
          increaseRecharge({phoneNumber: this.clickData.phoneNumber,rechargeAmount: value}).then(res => {
            if(res.flag){
              this.$emit('uploadList')
            }
          })
